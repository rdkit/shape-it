...
parseCommandLine(int argc, char* argv[])
{
	...
	while((choice = getopt_long(argc, argv,"vhpr:d:s:o:f:", Arguments, &optionIndex )) != -1)
	{
		switch (choice)
		{
            ...
            o.refInpReader = new OpenBabel::OBConversion();
			if (format.empty())
            	o.refInpReader->SetInFormat(o.refInpReader->FormatFromExt(optarg));
			else
				o.refInpReader->SetInFormat(o.refInpReader->FindFormat(format));
            break;
            ...
            o.dbInpReader = new OpenBabel::OBConversion();
			if (format.empty())
            	o.dbInpReader->SetInFormat(o.dbInpReader->FormatFromExt(optarg));
			else
				o.dbInpReader->SetInFormat(o.dbInpReader->FindFormat(format));
            break;
			...
			if (format.empty())
				...
				o.molOutWriter->SetOutFormat(o.molOutWriter->FindFormat(format));
            break;

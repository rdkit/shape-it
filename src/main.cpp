...
{  
	...
	OpenBabel::OBMol refMol;
   	(uo.refInpReader)->Read(&refMol, uo.refInpStream);
	std::string refName = "";
   	refName = refMol.GetTitle();
   	if (refName == "") { refName = "Unnamed_ref"; }
	...
	if (uo.showRef && !uo.molOutFile.empty())
   	{
      uo.molOutWriter->Write(&refMol, uo.molOutStream);
	}
	...
	bestSolution.refRotation = refVolume.rotation;
   	...
   	while ((uo.dbInpReader)->Read(&dbMol))
   	{
      	...
      	if (uo.bestHits || !uo.molOutFile.empty())
      	{
         	...
         	{
            	(uo.molOutWriter)->Write(&(bestSolution.dbMol), uo.molOutStream);
         	}
      	}	
